...
module PostsHelper
  def render_collect_btn post
    ...
    return button_to "收藏",add_collect_post_path(@post), :class => "btn btn-success", :remote => true, :method => :put, :id => "btn-collect" unless current_user
  ...
  def render_follow_btn user
    ...
    return button_to "關注",follow_user_path(user), :class => "btn btn-info", :remote => true, :method => :put, :id => "btn-follow" unless current_user
